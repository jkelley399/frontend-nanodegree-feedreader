...
- [Instructions](#instructions)
    ...
    - [How to Run the Application](#how-to-run-the-application)
        - [How to Install the Build Tools](#how-to-install-the-build-tools)
...
- ["PROJECT SPECIFICATION, Feed Reader Testing](https://review.udacity.com/#!/rubrics/18/view), and
...
- in index.html,
    ...
    - [Handlebars](https://cdnjs.cloudflare.com/ajax/libs/handlebars.js/4.0.12/handlebars.min.js), and
...
##### npm
-npm hould be installed with Node.js.
-Check current version: npm -v
-It is frequently updated, so it is recommended that you update it:
...
That command will ask some questions and then create the package.json file.
...
- Again, after confirming that you're in your project root, install Gulp and simultaneously make Gulp a development dependency (one of the "devDependencies") in package.json (
    ...
    - To confirm installation, run `gulp -v`
...
- Next, install the other Gulp dependencies required using the same npm techniques, making autoprefixer and eslint additional devDependencies (once more, be sure you're in your project root):
...
- When you are finished, ESLint will create a configuration file inside your `eslint` directory that will specify the set of rules to be followed.  As noted in ["Configuring ESLint"](https://eslint.org/docs/2.0.0/user-guide/configuring), the ESLint configuration files can take different forms:
...
- Alternatively, one can configure ESLint not to produce certain kinds of warnings.  For example, because this project is intended to run under node, gulpfile.json has a comment at the top of the file,

        /*eslint-env node */

    -   That statement notifies ESLint only to turn off Node.js warnings in this file.  While this type of comment acts like a configuration setting, it is
    located in and only affects the file in which it is placed.

- For more details on how to configure ESlint, *see*:
    - https://eslint.org/docs/user-guide/getting-started

        - In particular, you might want to pay special attention to the discussion beginning at approximately 4:00 of the "Getting Started Tutorial" video, which is also available on YouTube at: https://www.youtube.com/watch?time_continue=258&v=hppJw2REb8g
...
- At present, the project must be installed locally to run.  After installing all of the required build tools, proceed to download the project code from GitHub.
...
- After you have created the project environment by installing all of the required build tools, you will need to download the project code from GitHub:
    ...
    - install the cloned or downloaded repository locally in the project directory root;

- Once project code has been downloaded into the project directory root, run the application, run the application by using Gulp:
...
- For example, in the macOS operating system, the sequence of terminal commands might look like this and produce the following output in the terminal:

```
$ cd ~/frontend-nanodegree-feedreader
$ gulp
[19:12:51] Using gulpfile ~/miniconda3/envs/baseNodeJSFeedReader/frontend-nanodegree-feedreader/gulpfile.js
...
- There is a bug in the current implementation that requires additional manual intervention.  While Browsersync is serving files from `./dist, gulpfile.js` is not properly configured to run the Jasmine test file, `feedreader.js`.
- Therefore, place the cursor in the address bar and open a new file, in this case `index.html` in the root project directory.
- When this file is opened, `feedreader.js` will be triggered, and you should see:
...
![alt text](https://github.com/jkelley399/frontend-nanodegree-feedreader "Screenshot of frontend-nanodegree-feedreader/index.html showing 'Udacity Blog' and output from the Jasmine test suites with headline, `7 specs, 0 failures, randomized with seed 13945' ")
...
I have not tried to host this project at this time.

## Known Bugs or Implementation Problems

There are at least two known significant implementation problems, and one additional possible problem, at the present time:

1.  Variable test results
    1.  Although the application does appear to pass all of the Jasmine test suites, it is not doing so consistently.  Indeed, after considerable testing, the results of the Jasmine test suites appear to depend in the order in which they are called by Jasmine.
    ...
    2.  As presently configured, Jasmine runs the test suites in random order.  When the random setting is on, the application passes all of the test suites, very approximately, 50% of the time.
         1.  For example, running the application initially and then clicking on the reload button adjacent to the address bar in Chrome repeatedly produced 10 sets of results.
          2.  *See* the 10 screenshots arranged chronologically from Screen Shot 2019-07-01 at 1.59.17 PM to Screen Shot 2019-07-01 at 2.00.53 PM in
          https://github.com/jkelley399/frontend-nanodegree-feedreader
    3.  On the other hand, if the run test randomly option is turned off in Jasmine, the application seems to fail the Jasmine test suits fairly consistently.
          1.  *See* the 2 screenshots arranged chronologically from Screen Shot 2019-07-01 at 2.01.12 PM to Screen Shot Screen Shot 2019-07-01 at 2.01.49 PM in
          https://github.com/jkelley399/frontend-nanodegree-feedreader
    4.  The very variability of these test results has made it difficult to debug them.
2.  As discussed above (*see* [Running Locally](#running-locally)), there is a bug in the current implementation that requires additional manual intervention in order to run the application.
    1.  While Browsersync is serving files from `./dist, gulpfile.js`, it appears that there is some type of configuration error that prevents it from properly running the Jasmine test file, `./jasmine/spec/feedreader.js` automatically.
...
- I am particularly grateful for the considerable assistance that has been provided by Udacity mentor Peter J. three separate times both generally and specifically in connection with two test suites in feedreader.js:
    ...
    - For details on the assistance provided, please see the comments in `feedreader.js` [jasmine/spec/feedreader.js](https://github.com/jkelley399/frontend-nanodegree-feedreader/blob/master/jasmine/spec/feedreader.js)

-Additional materials relied upon are described in the:

    - "ADDITIONAL REFERENCES CONSULTED" section of [./gulpfile.js](https://github.com/jkelley399/frontend-nanodegree-feedreader/blob/master/gulpfile.js) and
    - "ADDITIONAL REFERENCES CONSULTED" section of [jasmine/spec/feedreader.js](https://github.com/jkelley399/frontend-nanodegree-feedreader/blob/master/jasmine/spec/feedreader.js)
...
1.  Test and possible fix the testing and automation environment and implementation
    ...
    4.  Try to find the proper location for `jasmine.json`.
        1.  It is currently in `./jasmine/spec/support`, but it
            ...
            ection](https://jasmine.github.io/setup/nodejs.html#configuration)
2.  Once the testing environment is working better, consider:
    ...
    2.  expanding the 'New Feed Selection' test suite with nested
            for loops.  *See*
                [commit 222f8c2a518f1fd980f9ff4482073e28b745476d]
...
6.  Explain and describe a conda installation.
